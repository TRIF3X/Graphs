...
while len(visited) < len(world.rooms):
    ...
    # bfs to find the closest '?'
        ...
        while queue:
            ...
            # if we find a '?' we will go ahead and travel that way
            if '?' in [room for move, room in visited[current_move[1]].items()]:
                for move, room in paths:
                    player.travel(move)
                    traversalPath.append(move)
                break
            # keep searching
            else:
                for move, room in visited[current_move[1]].items():
                    if room != start and room not in [a for b, a in paths]:
                        queue.append(list(paths) + [[move, room]])
